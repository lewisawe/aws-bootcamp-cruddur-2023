...
This script is used to load the tables 
...
The script used to list available created tables generated by schema-load
...
The seed script is for populating the table with data
...
def get_user_uuids():
  ...
  users = db.query_array_json(sql,{
    'my_handle':  'lewisawe',
    'other_handle': 'murenju'
  })
  my_user    = next((item for item in users if item["handle"] == 'lewisawe'), None)
  other_user = next((item for item in users if item["handle"] == 'murenju'), None)
  results = {
    'my_user': my_user,
...
Scan script is to scan the data that has been seeded
...
Drop Script is used to delete tables
